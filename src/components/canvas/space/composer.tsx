...
import { OrbitLine,Planet, PlanetClouds } from "./planet";
import { Star } from "./star";
import SpaceText from "./text";

export default function SpaceScene ({ sceneFOV }: { readonly sceneFOV: number }): React.ReactNode {
  ...
  const pointLight = useMemo(() => new THREE.PointLight(0xffffffff, 1, 0, 0), []);
  ...
  useFrame((state) => {
    ...
    if (planetRef.current && starRef.current) {
      orbitRefGroup.forEach((value) => {
        if (value.current) { value.current.visible = false; }
      });
      ...
      if (zoomOutRef.current < 0.25) {
        ...
        orbitRefGroup.forEach((value) => {
          if (value.current) { value.current.visible = true; }
        });
  ...
  return (
    <>
      <PerspectiveCamera fov={sceneFOV} makeDefault />
      <Selection>
