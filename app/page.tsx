...
import SpaceScene from "@/components/canvas/space/composer";
import { Loader } from "@/components/canvas/space/util";
import Scroll from "@/helpers/components/Scroll";
import { isMobile } from "@/helpers/components/utils";
import styles from "@/Home.module.css";

// const SpaceScene = dynamic(() => import("@/components/canvas/space/composer"), { ssr: false, loading: () => <Loading /> });
const View = dynamic(() => import("@/components/canvas/View").then((mod) => mod.View), { ssr: false });

export default function Page (): React.ReactNode {
  const isClient = () => typeof window !== "undefined";

  const [sceneFOV, setSceneFOV] = useState(60);

  useEffect(() => {
    if (!isClient()) return;

    isMobile.any() ? setSceneFOV(105) : setSceneFOV(60);
  }, []);

  return (
    <div>
      <Scroll>
        <View orbit={false} className={styles.background3D}>
          <Suspense fallback={<Loader styles={styles}/>}>
            <SpaceScene sceneFOV={sceneFOV}/>
          </Suspense>
        </View>
      </Scroll>
